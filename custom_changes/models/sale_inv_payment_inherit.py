...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    def create_invoices(self):
        ...
        sale_orders = self.env['sale.order'].browse(ctx.get('active_ids', []))

        if self.advance_payment_method == 'delivered':
            sale_orders.with_context(ctx)._create_invoices(final=self.deduct_down_payments)
...
class SaleOrderInherit(models.Model):
    ...
    def _create_invoices(self, grouped=False, final=False):
        ...
        for order in self:
            ...
            invoice_vals = order.with_context(self._context)._prepare_invoice()
    ...
    def _prepare_invoice(self):
        ...
        if self._context.get('folio_id'):
            invoice_vals.update({'folio_id': self._context.get('folio_id')})
